...
plot.co2.ts.trans.comp <- co2.ts.trans.comp  %>% components() %>% select("remainder") %>%
  ggplot(aes(x=remainder)) +geom_histogram() + 
...
plot.resid.co2.ts.trans.comp <- co2.ts.trans.comp  %>% components() %>% select("remainder") %>%
  autoplot() + ggtitle("Residuals Subsequent to Box-Cox Transformation")

grid.arrange(plot.resid.co2.ts.components, plot.resid.co2.ts.trans.comp, nrow=2)
```
...
```{r}
part.6 <- predict(model.arima.best.NSA, h=500)
part.6$pred
```



Lets first plot forecast for 5500 future intervals which would cover 420 and 500 ppm levels for irst and final times for 95% conf interval
```{r}
...
```{r}
# Get forcast for next 5500 weeks, suffucuent for lower 95% CI to pass 500 mark
model.arima.best.NSA.predict.ft <- forecast(model.arima.best.NSA, h=5500 )

# convert to ts object to find time points in future
model.arima.best.NSA.predict.ts <- as_tsibble(ts(model.arima.best.NSA.predict.ft$mean, start = 2017.99905280457 , frequency = 52.18 ))

# find all data point which cross 420 or 500 ppm values
model.arima.best.NSA.predict.tbl <- model.arima.best.NSA.predict.ft %>%
  as_tibble() %>%
  mutate(PF.420=`Point Forecast` >= 420, 
         Lo95.420=`Lo 95` >=420,
         Hi95.420=`Hi 95` >=420,
         Lo80.420=`Lo 80` >=420,
         Hi80.420=`Hi 80` >=420,
...
findForecast <- function(func, val){
...
# 420
PF.420.first <- findForecast(head, model.arima.best.NSA.predict.tbl$PF.420)
PF.420.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$PF.420)

Lo95.420.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Lo95.420)
Lo95.420.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Lo95.420)

Hi95.420.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Hi95.420)
Hi95.420.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Hi95.420)

Lo80.420.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Lo80.420)
Lo80.420.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Lo80.420)

Hi80.420.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Hi80.420)
Hi80.420.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Hi80.420)

#500
PF.500.first <- findForecast(head, model.arima.best.NSA.predict.tbl$PF.500)
PF.500.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$PF.500)
...
Hi95.500.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Hi95.500)
Hi95.500.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Hi95.500)

Lo80.500.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Lo80.500)
Lo80.500.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Lo80.500)

Hi80.500.first <- findForecast(head, model.arima.best.NSA.predict.tbl$Hi80.500)
Hi80.500.last <- findForecast(tail, model.arima.best.NSA.predict.tbl$Hi80.500)

predict.420=list(Lo95.420.first, 
                 Lo95.420.last, 
                 Lo80.420.first, 
                 Lo80.420.last, 
                 PF.420.first, 
                 ...
                 Hi95.420.first,
                 Hi95.420.last)
predict.500=list(Lo95.500.first, 
                 Lo95.500.last, 
                 Lo80.500.first, 
                 Lo80.500.last, 
                 PF.500.first, 
...
Also note that if we keep forcasting any further, the 95% CI forcast turns downwards and may touch 420ppm again, so above table would change if we take even wider forcast window.
...
forecast(model.arima.best.NSA, xreg=co2.noaa.weekly.ts.2100 ) 
```
