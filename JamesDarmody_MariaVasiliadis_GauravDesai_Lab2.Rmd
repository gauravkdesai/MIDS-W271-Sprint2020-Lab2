...
```
We can see a smoother line with the seasonally adjusted series. 
...
funFitModel <- function(param.row){
  ...
  #print(paste(p,q,d,P,Q,D))
  tryCatch({
  model.fit = Arima(y=as.ts(co2.noaa.weekly.ts.SA.training), order=c(p,d,q), seasonal=c(P,D,Q), lambda=lambda, include.drift = FALSE);
  model.info = data.frame(p,q,d,P,Q,D, model.fit$aic, model.fit$aicc, model.fit$bic);
...
funFitModel <- function(param.row){
  ...
  #print(paste(p,q,d,P,Q,D))
  tryCatch({
  model.fit = Arima(y=as.ts(co2.noaa.weekly.ts.training), order=c(p,d,q), seasonal=c(P,D,Q));
  model.info = data.frame(p,q,d,P,Q,D, model.fit$aic, model.fit$aicc, model.fit$bic);
