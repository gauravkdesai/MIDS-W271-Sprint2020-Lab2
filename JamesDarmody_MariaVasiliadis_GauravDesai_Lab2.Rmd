...
```{r}
week.frequency = 365/7
co2.noaa.weekly.ts.raw <- ts(data=co2.noaa.weekly.df$record.value,start = 1974.380	, frequency = week.frequency)
co2.noaa.weekly.ts <- as_tsibble(ts(data=co2.noaa.weekly.df$record.value,start = 1974.380	, frequency = week.frequency), class="matrix")
...
```{r}
co2.noaa.weekly.ts%>%
  as.ts(frequency=week.frequency) %>%
  decompose(type = "multiplicative") %>% # Additive gave better remainder centered around 0 over multiplicative
  autoplot()

```

Now lets derive seasonally adjusted (SA) series

```{r}
co2.noaa.weekly.ts.components <- co2.noaa.weekly.ts%>%
  as.ts(frequency=week.frequency) %>%
  decompose(type = "additive")

co2.noaa.weekly.ts.SA <- as_tsibble(as.ts(co2.noaa.weekly.ts,frequency=week.frequency) - co2.noaa.weekly.ts.components$seasonal)

# check head and tail to verify the duration matches original Time Series
cbind(head(co2.noaa.weekly.ts),tail(co2.noaa.weekly.ts))
cbind(head(co2.noaa.weekly.ts.SA),tail(co2.noaa.weekly.ts.SA))
