...
There are no missing values. Histogram of value is left skewed but that may not be correct representation of the co2 emission levels. So lets look at it from time dimension.
...
The ACF tells us that there is autcorrelation even after 24 months. On the other hand PACF drops after 2 but again at multples of 12.
...
ggplot(data=co2.ts.components)+
  ...
  ggtitle("Trend")
```
There are minor bumps and dips but there is no major shocks or sudden movements in trend.

Now lets look at seasons
...
```
We can clearly see that CO2 emission is higher in April-May and goes down in September-October every year. This could be due to summer in Northern himisphere vs southern hemisphere. Population, vegitation and other attributes of these two halfs of earth are quite different which could potentially cause this seasonal effect.

Now lets look at same data from another angel.
```{r}
co2.ts %>%
...
```
It is more or less same observation that northern summer increases CO2 emission and southern summer decreases CO2 eission within a year. But we can cearly see that CO2 emission is on the rise year on year.
