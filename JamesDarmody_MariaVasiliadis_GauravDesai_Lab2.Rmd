...
```
We can see a smoother line with the seasonally adjusted series. The acf chart shows a non-decreasing lag for every value. The pacf chart would suggest a non-stationary random walk process. 
...
funFitModel <- function(param.row){
  ...
  #print(paste(p,q,d,P,Q,D))
  tryCatch({
  model.fit = Arima(y=as.ts(co2.noaa.weekly.ts.SA.training), order=c(p,d,q), seasonal=c(P,D,Q), include.drift = FALSE);
  model.info = data.frame(p,q,d,P,Q,D, model.fit$aic, model.fit$aicc, model.fit$bic);
...
model.fit.info.df.SA %>%
  arrange(model.fit.bic) %>%
  head()

model.arima.best.SA = Arima(y=as.ts(co2.noaa.weekly.ts.SA.training), order=c(2,2,1), seasonal=c(0,0,0), include.drift = FALSE);
...
#Fit cubic model
co2.noaa.weekly.ts.SA.cubicmodel <- lm(formula = co2.noaa.weekly.ts.SA$value ~ time.index + I(time.index^2) + I(time.index^3), data=co2.noaa.weekly.ts.SA)
